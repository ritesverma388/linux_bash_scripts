...
q2="select id, key, name, value, configuration_id from connector_metadata_text where configuration_id in (select connector_configuration_id from  bi_metadata_connector where connector_id in (select id from connector_metadata_connector where name like '%Tableau OCF Connector%')) and key='tableau_online_site_id';"
...
q3="select id, key, name, value, configuration_id from connector_metadata_text where configuration_id in (select connector_configuration_id from  bi_metadata_connector where connector_id in (select id from connector_metadata_connector where name like '%Tableau OCF Connector%')) and key='server_uri';"
q4="select id, key, name, value, configuration_id from connector_metadata_boolean where configuration_id in (select connector_configuration_id from  bi_metadata_connector where connector_id in (select id from connector_metadata_connector where name like '%Tableau OCF Connector%')) and key='is_tableau_online';"
q5="select * from connector_metadata_connector where name like '%Tableau OCF Connector%';"
q6="select * from connector_metadata_agent where id in (select agent_id from connector_metadata_connector where name like '%Tableau OCF Connector%' and agent_id != NULL);"
